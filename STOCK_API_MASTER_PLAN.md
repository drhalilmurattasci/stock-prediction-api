...
- **Baselines:** `statsmodels` and `pmdarima` for ARIMA/SARIMA; **Nixtla StatsForecast** for fast, parallel classical models (AutoARIMA, ETS, Theta) at scale.
...
- **Deep learning:** **Nixtla NeuralForecast** and **Darts / PyTorch Forecasting** for **TFT** and **N-HiTS** when a symbol/regime justifies a trained model; these provide quantile/probabilistic outputs we expose as prediction intervals.
- **Tabular:** **XGBoost** and **LightGBM** over engineered features (lagged returns, volatility, calendar, cross-sectional, fundamental, and sentiment features) for direction/return regression and ranking.
- **Indicators / features:** **pandas-ta** as the default indicator library, with **TA-Lib** where its C-optimized implementations or specific functions are needed.
...
**MLflow** is the single source of truth for experiments and the model registry. Every training run logs params, metrics (per-horizon MAE/MASE/pinball loss, directional accuracy), artifacts, and the resolved feature set; promotion to `Staging`/`Production` in the MLflow Model Registry is what the serving layer reads to decide which model version is live. The MLflow backend store is Postgres; artifacts go to S3-compatible object storage (MinIO on-VPS, cloud bucket later). **Feast** is the optional feature store: we adopt it once online/offline feature skew and reuse across models become a real pain, backed by Postgres/Timescale offline and Redis online — not before.

#### Backtesting

**vectorbt** for fast, vectorized strategy and signal backtests, run as Celery jobs. Bias-free discipline is enforced as a hard rule, not a guideline: strictly point-in-time data (see Sharadar below for fundamentals), all features computed with as-of timestamps, no lookahead in indicator windows, explicit train/validation/test splits with **walk-forward / purged** cross-validation, realistic transaction costs and slippage, and an embargo between train and test to prevent leakage. Backtest results are logged to MLflow alongside the model that produced the signals.
...
| API framework | FastAPI + Uvicorn/Gunicorn | Async, typed, high-throughput; production process management |
...
| Classical forecasting | StatsForecast, statsmodels, pmdarima | Fast, scalable ARIMA/ETS/Theta baselines |
| Deep forecasting | NeuralForecast, Darts/PyTorch Forecasting (TFT, N-HiTS) | Trained probabilistic models for hard regimes |
| Tabular ML | XGBoost, LightGBM | Feature-based direction/return prediction and ranking |
| Indicators | pandas-ta, TA-Lib | Standard + C-optimized technical features |
| Tracking/registry | MLflow (+ optional Feast) | Single source of truth for runs and live model version |
| Backtesting | vectorbt | Fast vectorized, bias-free walk-forward testing |
...
- **Gunicorn** — battle-tested WSGI/process manager used as a production worker supervisor. · **Role in our stack:** master process managing multiple Uvicorn workers (`-k uvicorn.workers.UvicornWorker`) for multi-core scaling and resilience. · **Key features:** pre-fork worker model, worker lifecycle/health management, configurable timeouts and graceful restarts, signal-based zero-downtime reloads.
...
- **python-jose / PyJWT** — JSON Web Token encode/decode libraries for stateless auth. · **Role in our stack:** issue and verify signed access tokens behind FastAPI security dependencies (OAuth2 bearer). · **Key features:** HS256/RS256 signing and verification, `exp`/`iat`/`aud` claim validation, JWK support (python-jose), lightweight decode-only flows (PyJWT).
