...
- **Implemented immutable market and calibration evidence (migration head `0015_calibration_evidence`)** keeps raw daily bars plus append-only restatement history and exact later publication receipts; content-addressed complete split/dividend collections plus later receipts; Decimal34 adjustment-factor sets binding both action receipts and every exact raw-bar version/receipt; a global immutable high-water chain for the separately authorized acquisition journal; and append-only fitted calibration sets plus descriptive-only held-out coverage releases reconstructed from exact cohort/run/outcome proofs. Adjusted prices are derived from published factor bits on read, never written back over raw history, and descriptive calibration evidence cannot activate serving.
