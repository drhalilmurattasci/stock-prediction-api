...
- **Implemented immutable market and calibration evidence (migration head `0016_selection_policy_fence`)** keeps raw daily bars plus append-only restatement history and exact later publication receipts; content-addressed complete split/dividend collections plus later receipts; Decimal34 adjustment-factor sets binding both action receipts and every exact raw-bar version/receipt; a global immutable high-water chain for the separately authorized acquisition journal; registered prospective selection-policy bytes with durable fit/held-out opportunity exclusion; and append-only fitted calibration sets plus descriptive-only held-out coverage releases reconstructed from exact cohort/run/outcome proofs. Adjusted prices are derived from published factor bits on read, never written back over raw history, and descriptive calibration evidence cannot activate serving.
...
Two-tier auth: **API keys** for programmatic/customer access (hashed at rest, scoped, per-key quotas) and **JWT** for interactive/session use. Rate limiting starts with the owned `/v1` prefix middleware (HMAC-derived identities, Redis-backed shared counters, memory for development, and fail-closed storage outages); **Kong** sits in front as the dedicated API gateway when we need centralized auth, quota tiers, analytics, and routing across multiple services.
...
| Auth/gateway | API keys + JWT, owned limiter -> Kong | Tiered access; fail-closed in-app enforcement, then a dedicated gateway |
...
- **Owned prefix-scoped limiter** — application middleware that meters every `/v1` route before authentication. · **Role in our stack:** per-IP/per-API-key throttling on product and prediction endpoints without literal-route matching gaps or raw-key storage identifiers. · **Key features:** HMAC-derived bucket identities, Redis-backed shared fixed windows (memory in development), fail-closed backend outages, `429` responses with `Retry-After`, and standard `RateLimit-*` headers.
